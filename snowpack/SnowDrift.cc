...
void SnowDrift::compSnowDrift(const CurrentMeteo& Mdata, SnowStation& Xdata, SurfaceFluxes& Sdata, double& forced_massErode) const
{
	...
	if (windward || alpine3d || erosion) {
		...
		if (forced_massErode < -Constants::eps2) {
			...
			cout << "[SNP] PSUM erode : " << std::setprecision(12) << -Mdata.psum << endl;
			cout << "[SNP] Legacy Erod : " << std::setprecision(12) << -forced_massErode << endl;
			cout << "[SNP] Mass Erod : " << std::setprecision(12) << massErode << endl;
		...
		if (massErode > Constants::eps) { // ... or take away massErode from top element - partial real erosion
			if (fabs(EMS[nE-1].L * EMS[nE-1].Rho - EMS[nE-1].M) > 0.001) {
				prn_msg(__FILE__, __LINE__, "wrn", Mdata.date, "[D] Inconsistent Mass:%lf   L*Rho:%lf", EMS[nE-1].M,EMS[nE-1].L*EMS[nE-1].Rho);
